            ...
            listView.setOnItemClickListener((parent, v, position, id) -> {
                try {
                    ...
                    Request request = new Request(() -> {
                        URLRequest urlRequest = new URLRequest("http://www.kma.go.kr/DFSROOT/POINT/DATA/leaf." + code + ".json.txt");
                        String string = urlRequest.sendRequest();
                        if(string != null) {
                            Intent lvIntent = new Intent(getApplicationContext(), DongActivity.class);
                            lvIntent.putExtra("jsonString", string);
                            startActivity(lvIntent);
                        }
                        else {
                            Toast.makeText(GuActivity.this, "오류가 발생했습니다! 인터넷 상태를 재확인 해주신 다음, 앱을 다시 실행해주세요!", Toast.LENGTH_LONG).show();
                        }
                    });
                    request.execute();
                }
